...
function applyI18n(){document.documentElement.lang=lang;qs('mlang').textContent=lang==='nl'?'EN':'NL';document.querySelectorAll('.manual-nl').forEach(e=>e.classList.toggle('hide',lang!=='nl'));document.querySelectorAll('.manual-en').forEach(e=>e.classList.toggle('hide',lang!=='en'));}
...
const personStatusOptions=[['aanwezig','✅ Aanwezig'],['verlof','🌴 Verlof'],['ziek','🤒 Ziek'],['te laat','⏱ Te laat'],['afwezig','❌ Afwezig']];function personStatusLabel(v){const nl={aanwezig:'✅ Aanwezig',verlof:'🌴 Verlof',ziek:'🤒 Ziek','te laat':'⏱ Te laat',afwezig:'❌ Afwezig'};const en={aanwezig:'✅ Present',verlof:'🌴 Leave',ziek:'🤒 Sick','te laat':'⏱ Late',afwezig:'❌ Absent'};return (lang==='en'?en:nl)[v]||v}
function personCounts(){return{present:persons.filter(p=>p.status==='aanwezig').length,leave:persons.filter(p=>p.status==='verlof').length,sick:persons.filter(p=>p.status==='ziek').length,late:persons.filter(p=>p.status==='te laat').length,absent:persons.filter(p=>p.status==='afwezig').length,total:persons.length}}
...
function renderPersonnel(){const list=qs('pers_list');if(!list)return;const c=personCounts(),st=personnelStatus();qs('pers_counts').innerHTML=[['Aanwezig',c.present],['Verlof',c.leave],['Ziek',c.sick],['Te laat',c.late],['Afwezig',c.absent]].map(x=>`<div class="stat"><small>${x[0]}</small><b>${x[1]}</b></div>`).join('');const badge=qs('pers_status');badge.textContent=st;badge.className='pill '+(st==='GOEDGEKEURD'?'approved':st==='ACTIE VEREIST'?'rejected':'warning');qs('pers_advice').textContent=st==='GOEDGEKEURD'?'Bezetting compleet.':st==='ACTIE VEREIST'?'Meerdere zieken/afwezigen: actie of vervanging nodig.':st==='GEEN TEAM'?'Voeg medewerkers toe aan de lijst.':'Controleer verlof, ziekmeldingen, te laat en afwezigen.';list.innerHTML=persons.length?persons.map((p,i)=>{const cls=p.status==='ziek'?'sick':p.status==='verlof'?'leave':p.status==='afwezig'?'absent':p.status==='te laat'?'late':'';return `<div class="person-card ${cls}"><div class="person-head"><b>${p.name}</b><button class="smallbtn" style="background:#dc2626" onclick="removePerson(${i})">Verwijderen</button></div><div class="person-select"><div><label>Status</label><select onchange="setPersonStatus(${i},this.value)">${personStatusOptions.map(o=>`<option value="${o[0]}" ${p.status===o[0]?'selected':''}>${personStatusLabel(o[0])}</option>`).join('')}</select></div><div><label>Opmerking / reden</label><input placeholder="Bijv. reden, vervanger of tijd" value="${(p.note||'').replace(/"/g,'&quot;')}" oninput="updatePersonNote(${i},this.value)"></div></div><div class="mini">Status: ${p.status}</div></div>`}).join(''):'<div class="item">Nog geen medewerkers toegevoegd.</div>'}
...
function currentPersonnel(){return{type:'Personeel',lead:val('pers_lead'),shift:val('pers_shift'),location:val('pers_loc'),email:val('pers_mail'),notes:val('pers_notes'),people:persons.map(p=>({...p})),counts:personCounts(),status:personnelStatus()}}
...
function personIcon(st){return st==='aanwezig'?'✅':st==='verlof'?'🌴':st==='ziek'?'🤒':st==='te laat'?'⏱':'❌'}
function absentPeople(p){return (p.people||[]).filter(x=>x.status&&x.status!=='aanwezig')}
function presentPeople(p){return (p.people||[]).filter(x=>!x.status||x.status==='aanwezig')}
function absentTextBlock(p){const a=absentPeople(p);return a.length?'\n\nAFWEZIG / NIET AANWEZIG:\n'+a.map(x=>personIcon(x.status)+' '+x.name+' - '+personStatusLabel(x.status)+(x.note?' - '+x.note:'')).join('\n'):'\n\nAFWEZIG / NIET AANWEZIG:\nGeen afwezigen gemeld.'}
function presentTextBlock(p){const a=presentPeople(p);return a.length?'\n\nAANWEZIG:\n'+a.map(x=>'✅ '+x.name+(x.note?' - '+x.note:'')).join('\n'):'\n\nAANWEZIG:\nGeen aanwezigen gemeld.'}
...
Afwezig: ${p.counts.absent}${presentTextBlock(p)}${absentTextBlock(p)}
